...
describe('Create Student', () => {
  ...
  it('should show error message when email is too short', () => {
	...
	it('should not show error message when email of right length', () => {
    const wrapper = mount(<CreateStudent />);
    wrapper.find('input').get(0).value = 'bobaddfsdfsfd';
    wrapper.find('button').simulate('click');
	...
	it('should create a student', (done) => {
		nock('http://fakehost.com')
			.post('/students', {email: 'bob@aol.com'})
			.reply(200, {id: 99, email: 'bob@aol.com'});


		const stub = sinon.stub();
		const wrapper = mount(<CreateStudent created={stub} host="http://fakehost.com"/>);
    ...
    wrapper.find('button').simulate('click');
		...
		setTimeout(() => {
			try{
			expect(stub.callCount).to.equal(1);
			expect(stub.getCall(0).args[0]).to.deep.equal({id: 99, email: 'bob@aol.com'});
